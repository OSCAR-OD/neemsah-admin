...
import UploadMedia from "../../../Components/Media/Upload/UploadFooterMedia";
import MediaList from "../../../Components/Modal/Footer/FooterManageModal";
function FooterListPage() {
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/footer/deleteFooterNav/${deleteId}`);
    if (res?.success) {
      handleHideDelete();
      manageHideModal();
      toast.success("Footer Nav Link Deleted");
  ...
  const handleUploadSubmit = async (data) => {
    ...
    const res = await postApiHandler(
      editShow
        ? `/admin/footer/editFooterNav/${editShowId}`
        : `/admin/footer/addFooterNav`,
      {
        name: data?.name,
        slug: data?.slug,
    ...
    if (res?.success) {
      ...
      setShouldFetch((prev) => !prev);
      toast.success("Footer Nav Link Added");
    } else {
      toast.warn("Something went wrong");
    }
  };
      ...
      const handleFormSubmit = async (data) => {
          ...
          if (res?.success) {
            manageHideModal();
            setShouldFetch((prev) => !prev);
            toast.success(editShow ? "Footer Nav Link Updated" : "Footer Nav Link Added");
          } else {
            if (res?.response?.status === 404) {
              toast.warn("Data not found");
            } else {
  ...
  const getFetchList = async () => {
    ...
    const res = await getApiHandler("admin/footer/allFooterNav");
    //console.log("res", res);
    if (res?.success) {
    ...
    <Box>
      ...
      >
        ...
        <Modal
          ...
          title={editShow ? "Edit Footer Nav Link" : "Add Footer Nav Link"}
      ...
      <DeleteAlertModal
        isOpen={deleteShow}
        modifyText={"Footer Nav Link Deleted"}
