...
import MediaEmployeeList from "../../../Components/Media/List/MediaEmployeeList";
...
function EmployeeListPage() {
  ...
  const handleAttachmentsSubmit = async (data, selectId) => {
    if (selectId?.length > 0) {
      const res = await postApiHandler(
        editShow ? `/admin/hrm/editHrmEmployee/${editShowId}` 
        : `/admin/hrm/registerEmployee`,
        {
        name: data?.name,
        image: selectId[0],
      ...
      if (res?.success) {
        ...
        toast.success(editShow ? "Employee Info Updated" : "Employee Info Added");
      } else {
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/hrm/deleteHrmEmployee/${deleteId}`);
    if (res?.success) {
      handleHideDelete();
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler("/admin/hrm/allHrmEmployee");
  ...
  if (isLoading) {
    ...
    contentRender = fetchData?.map((item, index) => (
      <StyledTableRow key={item?._id}>
        ...
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.name || "N/A"}</Box>{" "}
        </StyledTableCell>
        ...
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.role || "N/A"}</Box>{" "}
        </StyledTableCell>
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.email || "N/A"}</Box>{" "}
        </StyledTableCell>
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.phone || "N/A"}</Box>{" "}
        </StyledTableCell>
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.address || "N/A"}</Box>{" "}
        </StyledTableCell>
  ...
  return (
    <Box>
      <PageHeader title={"Employee's List"} handleAddClick={uploadMediaShowModal} />
