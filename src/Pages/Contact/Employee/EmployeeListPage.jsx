...
function EmployeeListPage() {
  ...
  const handleAttachmentsSubmit = async (data, selectId) => {
    if (selectId?.length > 0) {
      const res = await postApiHandler(
        editShow ? `/admin/service/editEmployeeCard/${editShowId}` : `/admin/service/employee-card-add`,
        {
        name: data?.name,
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/service/deleteEmployeeCard/${deleteId}`);
    if (res?.success) {
      handleHideDelete();
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler("/admin/service/allEmployeeCard", {
      position: "cform",
    });
  ...
  return (
    <Box>
      <PageHeader title={"Contact Page Employee's List"} handleAddClick={uploadMediaShowModal} />
