...
import MediaList from "../../../Components/Modal/Crm/UploadTaskModal";
...
function TasksListPage() {
  ...
  const handleAttachmentsSubmit = async (data, selectId) => {
    if (selectId?.length > 0) {
      const res = await postApiHandler(
        editShow ? `/admin/crm/service/editEmployeeCard/${editShowId}` 
        : `/admin/crm/service/employee-card-add`,
        {
        name: data?.name,
  ...
  return (
    <Box>
    <PageHeader title={"Task's List"} handleAddClick={uploadMediaShowModal} />
