...
import MediaList from "../../../Components/Modal/Principal/PrincipalManageModal";
import UploadMedia from "../../../Components/Media/Upload/UploadPrincipalsMedia";
function PrincipalsListPage() {
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/principals/delete/${deleteId}`);
    if (res?.success) {
      handleHideDelete();
  ...
  const handleFormSubmit = async (data, selectedAttachment) => {
    ...
    if (selectedAttachment?.length > 0) {
      ...
      const res = await postApiHandler(
        editShow
          ? `/admin/principals/editPrincipal/${editShowId}`
          : `/admin/principals/addPrincipal`,
        {
          name: data?.name,
          position: "principal",
      ...
      if (res?.success) {
        manageHideModal();
        setShouldFetch((prev) => !prev);
        toast.success(editShow ? "Principal Updated" : "Principal Added");
      } else {
        if (res?.response?.status === 404) {
          toast.warn("Data not found");
        } else {
  ...
  const handleUploadSubmit = async (data, uploadFiles) => {
    if (uploadFiles?.length > 0) {
      ...
      const res = await postApiHandler(`/admin/principals/uploadPrincipalTab`, formData);
  ...
  const getFetchList = async () => {
    ...
    const res = await getApiHandler("/admin/principals/allPrincipals");
    if (res?.success) {
  ...
  return (
    <Box>
      <PageHeader title={"Principals List"} handleAddClick={uploadMediaShowModal} handleAddClick2={uploadMediaShowModal2} />
 ...
 <Modal
        ...
        title={"Upload Principals Details"}
      ...
      <Modal
        ...
        title={editShow ? "Edit Principal" : "Add Principal"}
