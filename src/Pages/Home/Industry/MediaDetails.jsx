...
function MediaDetails({
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler(`/admin/home/industryCard/single/${id}`);
    //console.log("res", res);
    if (res?.success) {
  ...
  useEffect(() => {
    if (imageData) {
     setValue("iName", imageData.iName || "");
     setValue("icTitle", imageData.icTitle || "");
     setValue("icDescription", imageData.icDescription || "");
    }
  }, [imageData]);
              ...
              <Controller
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.iName ? red[700] : ""}
                    >
                      Industry Name
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.iName}
              ...
              />
                ...
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.icTitle ? red[700] : ""}
                    >
                      Industry Card Title
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.icTitle}
                      placeholder="Industry Card Title"
                    />
                    {!!formState.errors?.icTitle ? (
                      <FormHelperText error>
                        {errors?.icTitle?.message}
                      </FormHelperText>
                    ) : (
                      ""
                    )}
              ...
              <Controller
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.icDescription ? red[700] : ""}
                    >
                      Industry Card Description
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.icDescription}
                      placeholder="Industry Card Description"
                      multiline
                      minRows={3}
                      maxRows={10}
                    />
                    {!!formState.errors?.icDescription ? (
                      <FormHelperText error>
                        {errors?.icDescription?.message}
                      </FormHelperText>
                    ) : (
                      ""
                    )}
