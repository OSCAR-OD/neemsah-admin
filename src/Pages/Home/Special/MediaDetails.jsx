...
function MediaDetails({
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler(`/admin/home/specialCard/single/${id}`);
    //console.log("res", res);
    if (res?.success) {
  ...
  useEffect(() => {
    if (imageData) {
     setValue("scTitle", imageData.scTitle || "");
     setValue("scDescription", imageData.scDescription || "");
    }
  }, [imageData]);
            ...
            <Box>
                ...
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.scTitle ? red[700] : ""}
                    >
                      Special Service Title
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.scTitle}
                      placeholder="Special Service Card Title"
                    />
                    {!!formState.errors?.scTitle ? (
                      <FormHelperText error>
                        {errors?.scTitle?.message}
                      </FormHelperText>
                    ) : (
                      ""
                    )}
              ...
              <Controller
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.scDescription ? red[700] : ""}
                    >
                      Special Service Description
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.scDescription}
                      placeholder="Special Service Description"
                      multiline
                      minRows={3}
                      maxRows={10}
                    />
                    {!!formState.errors?.scDescription ? (
                      <FormHelperText error>
                        {errors?.scDescription?.message}
                      </FormHelperText>
                    ) : (
                      ""
                    )}
