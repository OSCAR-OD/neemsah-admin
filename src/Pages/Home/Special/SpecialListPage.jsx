...
function SpecialListPage() {
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/home/deleteSpecialCard/${deleteId}`);
    if (res?.success) {
      handleHideDelete();
      toast.success("Special Service Deleted");
  ...
  const handleAlertAction = async () => {
    ...
    const res = await deleteApiHandler(
      `/admin/home/deleteSpecialCard/${storeSelectedData}`
    );
    if (res?.success) {
  ...
  const handleUploadSubmit = async (data, uploadFiles) => {
    ...
    if (uploadFiles?.length > 0) {
      ...
      formData.append("scTitle", data?.scTitle ? data?.scTitle : "");
      formData.append("scDescription", data?.scDescription ? data?.scDescription : "");
      formData.append(
        "type",
        uploadFiles[0]?.file?.type?.includes("video") ? "video" : "image"
      );
      formData.append("files", uploadFiles[0]?.file);
      const res = await postApiHandler(`/admin/home/special-card-upload`, formData);
  ...
  const handleAttachmentSubmit = async (data, files) => {
    const formData = new FormData();
    formData.append("scTitle", data?.scTitle);
    formData.append("scDescription", data?.scDescription);
    formData.append("files", files[0]);
    const res = await postApiHandler(
      `/admin/home/editSpecialCard/${storeSelectedData}`,
      formData
    );
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler("/admin/home/allSpecialCard", {
      position: "home",
    });
  ...
  if (isLoading) {
    ...
    contentRender = fetchData?.map((item, index) => (
      <StyledTableRow key={item?._id}>
        ...
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.scTitle || "N/A"}</Box>{" "}
        </StyledTableCell>
        ...
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 200 }}>{item?.scDescription || "N/A"}</Box>{" "}
        </StyledTableCell>
  ...
  return (
    <Box>
      <PageHeader title={"Special Services List"} handleAddClick={uploadMediaShowModal} />
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell>Special Service Title</StyledTableCell>
              <StyledTableCell>Special Service Image</StyledTableCell>
              <StyledTableCell>Special Service Description</StyledTableCell>
      ...
      <Modal
        ...
        title={"Upload Special Service Details"}
      ...
      <DeleteAlertModal
        isOpen={deleteShow}
        modifyText={"Special Service Section"}
