...
function MediaDetails({
  ...
  const getMedia = async () => {
    ...
    const res = await getApiHandler(`/admin/files/single/${id}`);
    if (res?.success) {
  ...
  useEffect(() => {
    ...
    //Set input value
    setValue("title", imageData?.caption);
              ...
              {/* <Controller
                render={({ field, formState }) => (
                  <FormControl fullWidth variant="outlined" sx={{ mt: 2 }}>
                    <Typography
                      variant="formLabel"
                      color={!!formState.errors?.altText ? red[700] : ""}
                    >
                      Alt text
                    </Typography>
                    <TextField
                      {...field}
                      error={!!formState.errors?.altText}
                      placeholder="Image alter text"
                      multiline
                      minRows={3}
                      maxRows={10}
                    />
                    {!!formState.errors?.altText ? (
                      <FormHelperText error>
