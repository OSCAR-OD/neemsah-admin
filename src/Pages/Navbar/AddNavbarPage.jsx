...
import AmenitiesManageModal from "../../../Components/Modal/Amenities/AmenitiesManageModal";
...
//Tab Header
const tabHeaderData = [
  { id: 1, label: "Schedule Form", type: "booking" },
  { id: 2, label: "Enquiry Form", type: "enquiry" },
  { id: 3, label: "Appointment Form ", type: "appointment" },
  { id: 4, label: "Contact Form", type: "contact" },
];
...
function ContactListPage() {
  ...
  const handleDeleteAction = async () => {
    const res = await deleteApiHandler(`/admin/contact-us/delete/${deleteId}`);
    if (res?.success) {
  ...
  const getFetchList = async () => {
    ...
    const res = await getApiHandler("/admin/contact-us/all", {
      type: tabHeaderData[tabValue]?.type,
    });
    if (res?.success) {
  ...
  if (isLoading) {
    ...
    contentRender = fetchData?.map((row, index) => (
      <StyledTableRow key={row?._id}>
        ...
        <StyledTableCell>
          {" "}
          <Box sx={{ maxWidth: 170 }}>{row?.name || "N/A"}</Box>
        </StyledTableCell>
        <StyledTableCell>
          {" "}
          <Button
        ...
        <StyledTableCell>
          {row?.message ? (
            <Box sx={{ maxWidth: 200 }}>{row?.message}</Box>
          ) : (
            "N/A"
          )}
        </StyledTableCell>
